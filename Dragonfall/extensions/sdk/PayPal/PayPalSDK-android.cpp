...
void PayPalSDK::buy(std::string itemName,double price)
{
	cocos2d::JniMethodInfo t;
    if (cocos2d::JniHelper::getStaticMethodInfo(t, CLASS_NAME, "paypalBuy", "(Ljava/lang/String;D)V"))
    {
        jstring jitemKey = t.env->NewStringUTF(itemName.c_str());
        t.env->CallStaticVoidMethod(t.classID,t.methodID,jitemKey,price);
...
void PayPalSDK::onPayPalDone(std::string payment)
{
	if(m_listener)
	{
...
static int tolua_ext_paypal_buy(lua_State *tolua_S)
{
...
#endif
    {
    	...
    	double price = tolua_tonumber(tolua_S, 2, 0);
    	PayPalSDK::GetInstance()->buy(itemName,price);
        return 0;
    }
#ifndef TOLUA_RELEASE
tolua_lerror:
...
extern "C" {
    ...
    JNIEXPORT void JNICALL PayPalSDKSDK_NATIVE_FUNCTION(PayPalSDK_onPayPalDone)(JNIEnv*  env, jclass cls,jstring jPaymentJson) 
    {
        PayPalSDK::GetInstance()->onPayPalDone(cocos2d::JniHelper::jstring2string(jPaymentJson));
    }
