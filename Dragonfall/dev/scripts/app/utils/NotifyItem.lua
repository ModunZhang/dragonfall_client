...
m.__tostring = function(a)
    return table.concat(LuaUtils:table_map(a, function(k, v)
        ...
        if v.type == "items" then
            txt = string.format("%s x %s", Localize_item.item_name[v.name], GameUtils:formatNumber(v.count))
        elseif v.type == "resources" then
            txt = string.format("%s x %s", Localize.fight_reward[v.name], GameUtils:formatNumber(v.count))
        elseif v.type == "soldierMaterials" then
            txt = string.format("%s x %s", Localize.soldier_material[v.name], GameUtils:formatNumber(v.count))
        elseif v.type == "soldiers" then
            txt = string.format("%s x %s", Localize.soldier_name[v.name], GameUtils:formatNumber(v.count))
        end
        return k, txt
    end), ", ")
end
