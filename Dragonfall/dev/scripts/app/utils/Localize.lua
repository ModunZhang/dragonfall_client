...
local DRAGON_SKILL_EFFECTION = {
    ...
    ["hellFire"] = _("特技：每轮战斗开始前红龙喷出一道火柱，对敌方战斗力最高的1个单位，降低其24%的攻击，每级+3%（特技必须在龙的生命值大于0时才能使用）"),
    ["lightningStorm"] = _("每轮战斗开始前蓝龙随机召唤3道闪电攻击敌方单位，被攻击单位的攻击10%，可叠加，每级+1.3%（特技必须在龙的生命值大于0时才能使用）"),
    ["poisonNova"] = _("特技：每轮战斗开始前绿龙释放一片毒雾，使敌方所有单位攻击降低8%的攻击，每级+1%（特技必须在龙的生命值大于0时才能使用）"),
    ["surge"] = _("减少总行军时间"),
}
