...
function WidgetPlayerNode:GetBasicInfoItemNode(basic_data)
	...
	local vip_bg  = display.newSprite("player_VIP_bg_126x44.png")
		:align(display.LEFT_BOTTOM,(self:GetSize().width - 560)/2, 0)
		:addTo(node)
	local icon_bg = UIKit:GetPlayerCommonIcon(data_source.playerIcon)
		:align(display.LEFT_BOTTOM,(self:GetSize().width - 560)/2 + 12,vip_bg:getPositionY()+vip_bg:getContentSize().height - 10)
		:addTo(node,-1)
	...
	if self:CallDelegate_(DELEGATE_METHODS.PlayerCanClickedButton,{"PlayerIcon"}) then
		display.newSprite("goods_26x26.png"):align(display.LEFT_BOTTOM, icon_bg:getPositionX()+6, icon_bg:getPositionY()+4):addTo(node)
	...
	UIKit:ttfLabel({
		...
		size = 18,
		color= 0xe19319
	}):align(display.CENTER, 63, 22):addTo(vip_bg)
