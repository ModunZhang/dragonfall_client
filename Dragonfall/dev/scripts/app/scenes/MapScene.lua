...
function MapScene:OnUserDataChanged_dragons(userData, deltaData)
    for k,dragon in pairs(userData.dragons) do
        ...
        if ok and self.dragonsLevel[k] ~= value and value > 1 then
            print(ok, self.dragonsLevel[k], value)
            ...
            if not GameDefautlt:IsPassedTriggerTips("warning") then
                ...
                if UIKit:GetUIInstance("GameUINpc") then
                    ...
                    GameUINpc:PromiseOfSay({
                        npc = "woman",
            words = _("领主大人，您的城市即将被攻击！您可以在城墙选择驻防部队统计敌人，或是撤下部队，避免无所谓的死伤（但您的资源将会被掠夺）")
                    }):next(function()
