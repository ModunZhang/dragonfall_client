...
def getAllArgs():
	...
	Logging.debug(PVRTOOL)
	Logging.debug(CONVERTTOOL)
	Logging.debug("------------End Config------------")

def CompileResources(in_file_path,out_dir_path):
	comand = "%s -i %s -o %s -ek %s -es %s" % (RES_COMPILE_TOOL,in_file_path,out_dir_path,XXTEAKey,XXTEASign)
	if QUIET_MODE:comand = "%s -q" % comand
		...
		if os.path.isfile(sourceFile):
			...
			if "_Compressed" == dir_name: #_Compressed文件夹
				...
				for image_file in os.listdir(sourceFile):
					...
					command = 'TexturePacker --format cocos2d --no-trim --disable-rotation --texture-format pvr2 --premultiply-alpha --opt PVRTC4 --padding 0 %s --sheet %s --data %s/tmp.plist' % (image_sourceFile,temp_file,TEMP_RES_DIR)
					executeCommand(command)
					finallyTemp = os.path.join(TEMP_RES_DIR,fileName + "_PVR_PNG.png")
					shutil.copy(temp_file,finallyTemp)
					if NEED_ENCRYPT_RES:
						CompileResources(finallyTemp,image_outdir)
						shutil.move(os.path.join(image_outdir,fileName + "_PVR_PNG.png"),image_targetFile)
					else:
...
if __name__=="__main__":
	if len(sys.argv) > 1:NEED_ENCRYPT_RES = sys.argv[1]
	getAllArgs()
