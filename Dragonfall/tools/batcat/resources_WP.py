...
def DXTFormatResources(in_file_path,out_file_path):
	comand = "%s -file %s -fileformat dds %s /rescalemode nearest /mipMode None /out %s" % (DXT_COMPRESS_TOOL,in_file_path,DXT_FOMAT,out_file_path)
	if QUIET_MODE:comand = "%s /quiet" % comand
	code,ret = executeCommand(comand,QUIET_MODE)
	return code == 0
