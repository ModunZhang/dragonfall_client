...
def exportScriptsEncrypt():
	...
	if NEED_ENCRYPT_SCRIPTS:
		...
		comand = "%s -i %s -o %s -e xxtea_zip -ex lua -ek %s -es %s" % (SCRIPT_COMPILE_TOOL,SCRIPTS_SRC_DIR,tempfile,XXTEAKey,XXTEASign)
		...
		if DEBUG_BUILD_USE_LUA_FILE:
			...
			comand = "%s -i %s -o %s -ex lua" % (SCRIPT_COMPILE_TOOL,SCRIPTS_SRC_DIR,tempfile)
			if QUIET_MODE:comand = "%s -q" % comand
			executeCommand(comand,QUIET_MODE)
