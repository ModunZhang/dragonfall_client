...
"$ANDROID_NDK_ROOT"/ndk-build $ANDROID_NDK_BUILD_FLAGS -j4 BUGLY_VERSION=$SO_FILE_VERSION NDK_DEBUG=$NDK_DEBUG NDK_TOOLCHAIN_VERSION=4.8 -C "$APP_ANDROID_ROOT" $* \
"NDK_MODULE_PATH=${APP_ANDROID_ROOT}:${COCOS2DX_ROOT}:${COCOS2DX_ROOT}/cocos:${COCOS2DX_ROOT}/external:${COCOS2DX_ROOT}/cocos/scripting:${COCOS2DX_ROOT}/cocos/quick_libs"
