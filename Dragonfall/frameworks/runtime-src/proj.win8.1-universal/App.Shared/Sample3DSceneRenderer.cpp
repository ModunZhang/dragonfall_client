...
void Sample3DSceneRenderer::CreateWindowSizeDependentResources()
{
	...
	static const XMVECTORF32 eye ={ 0.0f, 0.0f, 500.f, 0.0f };
...
void Sample3DSceneRenderer::Render()
{
	...
	XMStoreFloat4x4(&m_constantBufferData.model, projection*view*model);
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	////设置光栅化状态，使其生效 
	//m_deviceResources->GetD3DDeviceContext()->RSSetState(m_rasterState);
	...
	// 加载两个着色器后，创建网格。
	float len = 100.f;
	auto createCubeTask = (createPSTask && createVSTask).then([this, len]() {

		// 加载网格顶点。每个顶点都有一个位置和一个颜色。
		static const VertexPositionColor cubeVertices[] = 
		{
			{ XMFLOAT3(-len, -len, -len), XMFLOAT3(0.0f, 0.0f, 0.0f) },
			{ XMFLOAT3(-len, -len, len), XMFLOAT3(0.0f, 0.0f, 1.0f) },
			{ XMFLOAT3(-len, len, -len), XMFLOAT3(0.0f, 1.0f, 0.0f) },
			{ XMFLOAT3(-len, len, len), XMFLOAT3(0.0f, 1.0f, 1.0f) },
			{ XMFLOAT3(len, -len, -len), XMFLOAT3(1.0f, 0.0f, 0.0f) },
			{ XMFLOAT3(len, -len, len), XMFLOAT3(1.0f, 0.0f, 1.0f) },
			{ XMFLOAT3(len, len, -len), XMFLOAT3(1.0f, 1.0f, 0.0f) },
			{ XMFLOAT3(len, len, len), XMFLOAT3(1.0f, 1.0f, 1.0f) },
		};
