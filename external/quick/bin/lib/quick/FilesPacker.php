...
class FilesPacker
{
    ...
    function __construct($config, $options)
    {
        ...
        //dannyhe 忽略plist 与 pkm(2dx etc压缩图片)
        $this->files_exclude_extesion = array("plist","pkm","ExportJson");
    }
    ...
    function run()
    {
        ...
        {
            $files[] = $this->config['srcpath'];
            $modules = $this->prepareOneFileForPack($this->config['srcpath']);
        }
    ...
    protected function prepareOneFileForPack($file)
    {
        $modules = array();
        $path_parts = pathinfo($file);
        $moduleName = str_replace('.', SPLIT_CHAR, $path_parts['basename']);
        $tempFilePath = $path_parts['dirname'] . DS . $moduleName . '.tmp';
           ...
           # else
            {
                ...
                //dannyhe 忽略指定后缀名的文件加密 只对单文件加密时生效
                if ($this->config['pack'] == self::COMPILE_FILES&&count($this->files_exclude_extesion)>0)
                {
                    foreach ($this->files_exclude_extesion as $ext) {
                        if (substr_count($moduleName,SPLIT_CHAR . $ext))
                        {
                            if (!$this->config['quiet'])
                            {
                                printf("  > ignore file [ %s ]\n",str_replace(SPLIT_CHAR,'.', $moduleName));
                            }
                            $skip = true;
                            break;
                        }
